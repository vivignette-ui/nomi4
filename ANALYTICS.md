...
## Storage (Sept 2026): Supabase primary, Airtable archive

Supabase project **nomi-analytics** (`bfaujtkwwvfhkuqkpxld`) is the live
store: tables `nomi_users` / `nomi_events` / `nomi_prompts` mirror the
Airtable tables column-for-column (see `supabase/schema.sql`) so pre- and
post-migration data compare directly. Airtable keeps everything written
before the cutover and is the automatic fallback if Supabase is unreachable
(the server also reads a user's Airtable history forward the first time it
sees them after cutover, so counters and once-only milestones continue).
Configured by `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` on Vercel;
RLS is enabled with no public policies, so only the server can read or write.

## Live experiment

See [EXPERIMENT.md](EXPERIMENT.md) for `nomi_personalization_onboarding_v3`:
flows, assignment, QA overrides, the full event schema, the off switch, and
ready-made SQL.
